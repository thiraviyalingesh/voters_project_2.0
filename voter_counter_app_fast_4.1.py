"""
Electoral Roll Voter Counter - GUI Application v6.0 (FAST VERSION - NO HOUSE NO)
...
v6.0 Features:
- REMOVED House Number extraction for SPEED optimization
- Only extracts: Name, Age, Gender (and Voter ID, Relation)
- House No column kept in Excel for compatibility (empty)
...
def ocr_single_card(args):
    ...
    try:
        ...
        # v5.3: Three-pass extraction for Name, Age, Gender (House No removed for speed)

        # v5.3: Three-pass extraction for Name
...
def enhanced_ocr_name_age_gender(args):
    """Worker function for enhanced OCR focused on Name, Age, Gender."""
...
def extract_cards_from_single_pdf(args):
    ...
    try:
        ...
        for page_num in range(start_page, end_page):
            ...
            # Batch: collect cards for this page
            page_cards = []
            ...
            for row in range(num_rows):
                for col in range(num_cols):
                    ...
                    # Fast brightness check using sampling (100 pixels instead of all)
            ...
            # Batch write: save all cards from this page at once
            for card_num, card_img in page_cards:
                card_filename = output_path / f"{card_num}.png"
                card_img.save(card_filename, "PNG", compress_level=1)  # Low compression = faster
    ...
    def batch_extract_thread(self, folder_path, resume_mode=False):
        ...
        if current_phase < 1:
            ...
            for pdf_name, (card_count, output_path) in pdf_card_info.items():
                if output_path:
                    ...
                    try:
                        jpg_files = [f for f in pdf_card_dir.iterdir() if f.suffix.lower() == '.png']
        ...
        try:
            with open(log_file, 'w', encoding='utf-8') as f:
                json.dump({
                    ...
                    'missing_name': phase2_missing_name,
                    'missing_age': phase2_missing_age,
                    'missing_gender': phase2_missing_gender,
        ...
        for global_idx in sorted(ocr_results.keys()):
            ...
            if data:
                ...
                # House No column kept for compatibility (not extracted)
            ...
            ws.cell(row=row_num, column=12, value=f"{s_no}.png")  # Card File
