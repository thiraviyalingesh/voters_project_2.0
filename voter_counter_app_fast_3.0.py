"""
Electoral Roll Voter Counter - GUI Application v3.0 (FAST VERSION)
...
- Fix missing Age/Gender from existing images
- Combine multiple constituency Excel files
"""
...
class VoterCounterApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Electoral Roll Voter Counter v3.0 (FAST - Batch)")
        self.root.geometry("850x800")
        self.root.resizable(True, True)

        # Use 7 parallel workers (optimized for 12 cores, 8GB RAM)
        # Each Tesseract process uses ~500-700MB RAM
        # 7 workers × 700MB = ~4.9GB + OS + App = ~6.5GB total
        # 12 cores available, but RAM is the bottleneck
        self.num_workers = 7
    ...
    def batch_extract_thread(self, folder_path, resume_mode=False):
        ...
        if current_phase < 2:
            ...
            if cards_to_ocr:
                ...
                batch_size = 100  # Submit 100 tasks at a time
        ...
        for global_idx in sorted(ocr_results.keys()):
            ...
            if data:
                ...
                age_cell = ws.cell(row=row_num, column=7, value=age_val)
                gender_cell = ws.cell(row=row_num, column=8, value=gender_val)
